...
void on_cell_clicked_sound(GtkWidget *button, gpointer user_data) {
    GstBus *bus;
    GstMessage *msg;
    GError *error = NULL;
...
void on_cell_clicked_sound2(GtkWidget *button, gpointer user_data) {
    GstBus *bus;
    GstMessage *msg;
    GError *error = NULL;
...
/*void sound3() {
    ...
    bus = gst_element_get_bus(pipeline3);
    msg = gst_bus_timed_pop_filtered(bus, GST_CLOCK_TIME_NONE, GST_MESSAGE_EOS | GST_MESSAGE_ERROR);
...
void on_cell_clicked_difficult(GtkWidget *widget, gpointer user_data) {
    ...
    while (data->is_player1_turn) {
        ...
        if (cell->is_ship) {
            ...
            on_cell_clicked_sound(GTK_WIDGET(widget), data);
        }
        else {
            // Missing
            ...
            on_cell_clicked_sound2(GTK_WIDGET(widget), data);
        }
...
/**********************************************                                             **********************************************/
	...
	// g_signal_connect(button, "clicked", G_CALLBACK(on_cell_clicked_sound), NULL);
...
void create_game_interface_difficult(GameData *data) {
    ...
    data->button_replace_flot = gtk_button_new_with_label("🔃⚓   REGROUPER LA FLOTTE   ⚓🔃\n");
...
void create_game_interface_average(GameData *data) {
    ...
    GtkWidget *button_box_down = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 10);
    gtk_container_add(GTK_CONTAINER(main_box), GTK_WIDGET(button_box_down));
    gtk_box_set_spacing(GTK_BOX(button_box_down), 900);
...
void on_button_difficult_clicked(GtkWidget *button, gpointer user_data) {
    ...
    gst_init(NULL, NULL);
    // Creating a conveyor element for a sound effect
    pipeline = gst_parse_launch("playbin uri=file:///D:/!_GTK/battleship_v22/shot.wav", NULL);
    pipeline2 = gst_parse_launch("playbin uri=file:///D:/!_GTK/battleship_v22/miss.wav", NULL);
    //pipeline3 = gst_parse_launch("playbin uri=file:///D:/!_GTK/battleship_v22/pirate.wav", NULL);

    start_game_difficult();

    // Stopping and releasing conveyor resources
    gst_element_set_state(pipeline, GST_STATE_NULL);
